...
function LeadGenPage() {
  ...
  const handleGenerateLead = useCallback(() => {
    ga.event("generate_lead", { currency: "USD", value: 1 });
  }, [ga]);

  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      <h1 className="mb-6 text-3xl font-bold tracking-tight text-slate-900">Generate lead</h1>
      <p className="mb-6 text-slate-600 leading-relaxed">
        This page demonstrates the{" "}
        <code className="rounded bg-slate-200 px-1.5 py-0.5 text-sm">generate_lead</code> event.
        Click the button below to send the event to Google Analytics.
      </p>
      <button
        type="button"
